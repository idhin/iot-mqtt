...
def update_graph_comp(n):
    ...
    dt = 0.02
    gyrpitch = df['gyrxMA'] * dt
    gyrroll = df['gyryMA'] * dt
    gyryaw = df['gyrzMA'] * dt
    df['gyrpitch'] = df['gyrxMA'] * dt
    df['gyrroll'] = (df['gyryMA'] * dt)
    ...
    figureC = {
        'data': [
            {'x': df['time'], 'y': df['accyaw'], 'type': 'line', 'name': 'Accel'},
            {'x': df['time'], 'y': df['gyryaw'], 'type': 'line', 'name': 'Gyro'},
            {'x': df['time'], 'y': df['yaw'], 'type': 'line', 'name': 'Complementary Filter'}
        ],
        'layout': {
            'width': 1300,
            'height': 300,
            'title': 'Complementary Filter',
            'yaxis': dict(title='Roll'),
            'xaxis': dict(autorange=True,
                          showgrid=False,
                          zeroline=False,
                          showline=False,
